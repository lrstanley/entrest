...
>
> Schemas embedding [`ent.View`](https://pkg.go.dev/entgo.io/ent#View) automatically expose only
> `read` and `list`. Requesting `create`, `update`, or `delete` on a view (or an edge on a view)
> fails codegen.
>

...
:::note[Reference]
Per-schema, field, and edge annotations that control what appears in the OpenAPI spec and generated
HTTP handler. Misplaced annotations fail at codegen — see [Troubleshooting](/entrest/resources/troubleshooting/#annotation-on-the-wrong-graph-level).
:::
...
| [WithEdgeEndpoint](#withedgeendpoint) | <Usage types={["schema", "edge"]} /> | Sets the edge to have an endpoint. |
...
| [WithIncludeOperations](#withincludeoperations) | <Usage types={["schema", "edge"]} /> | Includes the specified operations in the REST API for the schema or edge. Schema ops are inherited by edges unless overridden. |
| [WithExcludeOperations](#withexcludeoperations) | <Usage types={["schema", "edge"]} /> | Excludes the specified operations in the REST API for the schema or edge. |
...
This would effectively result in the following pseudo-code being used behind the scenes:
...
[ [pkg.go.dev](https://pkg.go.dev/github.com/lrstanley/entrest#WithEdgeEndpoint) | usage: <Usage types={["schema", "edge"]} /> ]

> Sets whether the edge should have a dedicated endpoint. If the edge is eager-loaded and
> [`Config.DisableEagerLoadedEndpoints`](https://pkg.go.dev/github.com/lrstanley/entrest#Config.DisableEagerLoadedEndpoints)
> is set, or if [`Config.DisableEdgeEndpoints`](https://pkg.go.dev/github.com/lrstanley/entrest#Config.DisableEdgeEndpoints)
> is set, this defaults to false. Not required unless endpoints are disabled globally/per-schema and you want to
> specifically enable one edge, or you want to disable an edge from having an endpoint.
>
> When applied on a schema, it becomes the default for all edges on that schema (individual edges can still override).
>
> See [Eager Loading](/entrest/openapi-specs/eager-loading/) for more information.
...
func (Pet) Annotations() []ent.Annotation {
    return []ent.Annotation{
        // Disable edge endpoints for all edges on Pet (override per-edge if needed).
        entrest.WithEdgeEndpoint(false),
    }
}
```
...
> build upon them.
>
> See [Mount the Handler](/entrest/http-handler/getting-started/) for more information.
...
> Includes the specified operations in the REST API. If unset, [`Config.DefaultOperations`](https://pkg.go.dev/github.com/lrstanley/entrest#Config.DefaultOperations)
> is used (or, for edges without an explicit setting, the parent schema's operations).
>
> - **Schema**: controls which entity CRUD endpoints are generated. Edges without their own
>   include/exclude operations inherit these settings (including dedicated edge endpoints and
>   create/update body fields).
> - **Edge**: overrides any inherited schema operations. Controls whether the edge appears in
>   create/update request bodies, and whether its dedicated GET endpoint is generated (`read`
>   for unique edges, `list` for non-unique).
>
> To keep edge endpoints when the schema has restricted operations, set include/exclude
> operations on the specific edges. To allow updating some edges while disallowing field edits,
> keep `OperationUpdate` on the schema, mark fields with [`WithReadOnly`](#withreadonly), and
> restrict individual edges as needed.
...
```go title="internal/database/schema/schema_pet.go" ins={5-8}
func (Pet) Edges() []ent.Edge {
    return []ent.Edge{
        edge.From("owner", User.Type).
            ...
            Unique().
            Annotations(
                // Override schema ops so this edge still has a GET endpoint and can be
                // updated, even if the schema excluded read/update.
                entrest.WithIncludeOperations(
...
> Excludes the specified operations in the REST API. If unset, [`Config.DefaultOperations`](https://pkg.go.dev/github.com/lrstanley/entrest#Config.DefaultOperations)
> is used (or, for edges without an explicit setting, the parent schema's operations). See
> [`WithIncludeOperations`](#withincludeoperations) for how schema vs edge operations interact.

...
> disable endpoints for edges which are also eager-loaded, or if [`Config.DisableEdgeEndpoints`](https://pkg.go.dev/github.com/lrstanley/entrest#Config.DisableEdgeEndpoints)
> is set to true, this will default to false. Not required to be provided unless endpoints are disabled
> globally and you want to specifically enable one edge to have an endpoint, or want to disable an edge

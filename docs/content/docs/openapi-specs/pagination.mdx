...
:::note[Explanation]
How list pagination works in entrest — query parameters, response envelope, and when to disable it.
For codegen defaults, see [Codegen Configuration — Pagination](/entrest/openapi-specs/configuration/#pagination).
:::
...
func main() {
    ...
    for {
        ...
        err = json.UnmarshalRead(resp.Body, &pageResults)
        if err != nil {
            panic(err)
        }
        resp.Body.Close()

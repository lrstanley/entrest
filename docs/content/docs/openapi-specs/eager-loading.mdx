...
:::note[Explanation]
When to eager-load edges, how entrest exposes them in responses and edge endpoints, and the
...
the edges are fetched along with the parent entity. This is primarily useful when you have a parent
...
- If you eager load an edge, by default the associated edge endpoint will have its pagination turned off.
  This can be turned off with the config option [`DisableEagerLoadNonPagedOpt`](https://pkg.go.dev/github.com/lrstanley/entrest#Config.DisableEagerLoadNonPagedOpt).
- If you only ever want to eager load a specific edge and not have a dedicated endpoint, you can use
  ...
  (eager-loaded edges only), or [`DisableEdgeEndpoints`](https://pkg.go.dev/github.com/lrstanley/entrest#Config.DisableEdgeEndpoints)
  (all edges). `WithEdgeEndpoint` can also be set on a schema to disable endpoints for all of its edges.

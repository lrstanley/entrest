...
func (_q *UserQuery) QueryFollowedPets() *PetQuery {
	query := (&PetClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *UserQuery) QueryFollowing() *FollowsQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withFollowedPets: _q.withFollowedPets.Clone(),
...
func (_q *UserQuery) WithFollowedPets(opts ...func(*PetQuery)) *UserQuery {
	query := (&PetClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withFollowedPets = query
	return _q
}
...
func (_q *UserQuery) WithFollowing(opts ...func(*FollowsQuery)) *UserQuery {
	query := (&FollowsClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withFollowedPets; query != nil {
		if err := _q.loadFollowedPets(ctx, query, nodes,
...
func (_q *UserQuery) loadFollowedPets(ctx context.Context, query *PetQuery, nodes []*User, init func(*User), assign func(*User, *Pet)) error {
...
func (_q *UserQuery) loadFollowing(ctx context.Context, query *FollowsQuery, nodes []*User, init func(*User), assign func(*User, *Follows)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
